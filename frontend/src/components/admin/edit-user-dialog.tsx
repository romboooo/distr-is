...
export function EditUserDialog({
  ...
  const onSubmit = async (data: EditUserFormData) => {
    try {
      if (
        (data.type === 'ADMIN' || data.type === 'PLATFORM') &&
        data.type !== user.type
      ) {
        if (currentUser.data?.type !== 'ADMIN') {
          toast.error(
            'Only ADMIN users can promote users to ADMIN or PLATFORM roles',
          );
          return;
        }
  ...
  const handleTypeChange = (value: string) => {
    ...
    if (
      (userType === 'ADMIN' || userType === 'PLATFORM') &&
      userType !== user.type
    ) {
      if (currentUser.data?.type !== 'ADMIN') {
        toast.error(
      ...
      <DialogContent className='sm:max-w-[425px]'>
        <DialogHeader>
          ...
          <div className='mt-1 text-muted-foreground text-sm'>
            Update user details. Note: Changing user type may require admin
            privileges.
          </div>
        </DialogHeader>
            ...
            <div className='col-span-full bg-yellow-50 p-3 border border-yellow-200 rounded-md'>
              <div className='flex items-start gap-2'>
                ...
                <p className='text-yellow-700 text-sm'>
                  <strong>Warning:</strong> You are about to change this user's
                  type to {selectedType}. This action requires admin privileges
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={updateMutation.isPending}
            className={
              showTypeWarning ? 'bg-yellow-500 hover:bg-yellow-600' : ''
            }
          >

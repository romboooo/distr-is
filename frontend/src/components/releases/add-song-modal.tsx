...
export function AddSongModal({ releaseId, artistId, open, onOpenChange, onSuccess }: AddSongModalProps) {
  ...
  const onSubmit = async (formData: SongFormData) => {
    ...
    try {
      ...
      const errMsg = error instanceof Error
        ? error.message
        : (error as AxiosError<ErrorResponse>)?.response?.data?.message || 'An error occurred while adding the song';
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            />
            {errors.musicAuthor && (
              <p className="text-destructive text-sm">{errors.musicAuthor.message}</p>
            )}
          </div>
          ...
          <div className="flex items-start space-x-2">
            ...
            <div className="gap-1.5 grid leading-none">
              <Label htmlFor="parentalAdvisory">
                Parental Advisory
              </Label>
                ...
                {selectedFile ? (
                  ...
                  <>
                    ...
                    <p className="max-w-[250px] font-medium text-center">
                      {isDragging ? 'Drop your audio file here' : 'Drag & drop or click to upload audio file'}
                    </p>
                    <p className="mt-1 text-muted-foreground text-sm">
                      <span className="font-semibold text-primary">Any audio format</span> • Max 50 MB
                    </p>
                    {isDragging && (
          ...
          <DialogFooter>
            ...
            <Button
              type="submit"
              disabled={isUploading || addSongMutation.isPending || uploadFileMutation.isPending || !selectedFile}
            >
              {isUploading ? (
                <>

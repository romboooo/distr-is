// src/components/releases/release-edit-content.tsx
...
export interface ReleaseEditContentProps {
  releaseId: number;
  onCancel?: () => void;
  onSuccess?: () => void;
  readOnly?: boolean;
...
export function ReleaseEditContent({
  releaseId,
  onCancel,
  onSuccess,
  readOnly = false,
}: ReleaseEditContentProps) {
  const navigate = useNavigate();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const { mutate: updateReleaseMutation, isPending: isUpdating } =
    useUpdateRelease();
  ...
  const form = useForm<ReleaseFormData>({
    ...
    values: {
      ...
      genre: release?.genre || '',
      releaseType:
        (release?.releaseType as ReleaseFormData['releaseType']) || 'SINGLE',
      date: release?.date
      ...
      <div className="mx-auto py-8 max-w-2xl container">
        <Alert variant="destructive">
          ...
          <Button
            variant="outline"
            className="mt-4"
            onClick={onCancel || (() => navigate({ to: '/artist/releases' }))}
          >
    ...
    return (
      <div className="mx-auto py-8 max-w-2xl container">
        <Alert variant="destructive">
          ...
          <AlertDescription>
            Only releases in DRAFT or WAITING_FOR_CHANGES state can be edited.
            Current state: {release.moderationState}
          </AlertDescription>
          <Button
            variant="outline"
            className="mt-4"
        ...
        <CardHeader className="flex flex-row justify-between items-start pt-4">
          <div>
            <CardTitle className="text-2xl">
              {readOnly ? 'View Release' : 'Edit Release'}
            </CardTitle>
            <CardDescription>
              {readOnly
                ? 'Release details'
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    onValueChange={(value) =>
                      form.setValue(
                        'releaseType',
                        value as ReleaseFormData['releaseType'],
                      )
                    }
        ...
        {!readOnly && (
          <CardFooter className="flex justify-between">
            ...
            <Button
              ...
              disabled={!isDirty || isUpdating || isSubmitting}
              className="gap-2"
            >
